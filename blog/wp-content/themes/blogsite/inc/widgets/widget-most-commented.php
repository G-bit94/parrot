...
class BlogSite_Most_Commented_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			...
			'description' => __( 'Display most commented posts with thumbnails.', 'blogsite' )
		);

		// Create the widget.
		parent::__construct(
			'blogsite-most-commented', // $this->id_base
			__( '&raquo; Most Commented Posts', 'blogsite' ), // $this->name
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Most Commented Posts', 'blogsite' ),
			'limit'     => 6,
			'day_limit' => 10000,			
			'show_number' => true
		);
	...
	?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'day_limit' ); ?>">
				<?php _e( 'Show posts published within xx days', 'blogsite' ); ?>
			</label>
