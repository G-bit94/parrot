...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (hash_equals($_SESSION['user']['csrf_token'], $_POST['csrf_token']) && $_SESSION['user']['email'] !== null) {
        ...
        if (isset($_POST['page'])) {
            ...
            if ($page == 'dashboard') {
                ...
                if ($rs->num_rows !== 0) {
                    foreach ($rs as $row) {
                        ...
                        ?>
                        <object>
                            <div class="position-relative col-md rounded-3 border shadow-sm" role="button"
                                onclick="fetchSingleItem('<?php echo $row['unique_id']; ?>');">
            ...
            if ($page == 'account') {
                ...
                if ($intent == 'search') {
                    ...
                    if (isset($_POST["type"])) {
                        $type = $_POST["type"];
                        $where = $where . " AND content_types.type = '$type'";
                    }
                        ...
                        <table class="table table-hover align-middle overflow-auto">
                            <tbody>
                                ...
                                foreach ($rs as $row) {
                                    ...
                                    <tr class="spacer" style="height: 6px;"><input type="hidden" name="keys_array[]"
                                            value="<?php echo $row["unique_id"]; ?>" /></tr>
                                    <tr class="rounded-3 card-body shadow-sm reg-rows">
                                        ...
                                        <td class="p-1 border-0" onclick="fetchSingleItem('<?php echo $row['unique_id']; ?>');">
                                            <strong><small><?php echo truncStringToNrstWord($text, 3); ?></small></strong>
                                        </td>
                                        <td class="p-1 border-0" onclick="fetchSingleItem('<?php echo $row['unique_id']; ?>');"><small
                                                class="mb-0"><?php echo truncStringToNrstWord($text, 15); ?></small></td>
                                        ...
                                        <td class="py-1 rounded-end border-0" type="button">
                                            <div class="form-check">
                                                <input class="form-check-input position-static" onclick="" name="uids_array[]" value=""
                                                    data-bs-toggle="tooltip" title="Click to approve" type="checkbox">
                                            </div>
                                        </td>
                                    </tr>
                                    <?php
    ...
    echo "<script type=\"text/javascript\">window.location.href = \"/\"</script>"; //redirect home
